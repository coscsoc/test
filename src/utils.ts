  ...
  function _readdir(dir: string) {
    ...
    files.forEach(async (file) => {
      ...
      if (state.isDirectory()) {
        ...
        // dir: mock\xx\xx\
        // file: xxx.ts
        // path.join(dir,file): mock\xx\xx\xxx.ts
        // path.relative: role\permission\index.ts
        let p = path.relative(mockDir, path.join(dir, file))
        // 使用POSIX /分隔符
        mockFiles.push(normalizePath(p));
      }
    });
  }
